...
def Test(net, testloader):
    ...
    confusion = [ [0 for j in range(5)] for i in range(5) ]
    with torch.no_grad():
        running_loss = 0
        cnt = 0
        acc = 0
        for i, _data in enumerate(testloader, 0):
            ...
            for j in range(len(outputs)):
                confusion[labels[j]][outputs[j]] += 1
            bar.update(1)
        acc = acc / cnt
    # print(confusion)
    return acc * 100
...
def ChangeState(net, train_acc, test_acc, target_acc, results):
    ...
    if test_acc > target_acc[net.name]:
        ...
        torch.save(net, 'results/weights/model-'+net.name+'.pth')
...
def TrainIter(net, epoches, trainloader, testloader, target_acc, results):
    ...
    net = torch.load('results/weights/model-'+net.name+'.pth')
    
    for epoch in range(epoches // 2):
        print('\033[38;5;014mepoch', epoches + epoch, '\033[0m')
        train_acc = Train(net, trainloader, loss_func, learning_rate / 10)
        test_acc = Test(net, testloader)
        highest_acc = max(highest_acc, test_acc)
        ChangeState(net, train_acc, test_acc, target_acc, results)
        print('train acc:', '{:.2f}'.format(train_acc), ' ', 'test acc:', '{:.2f}'.format(test_acc), ' ', '\033[38;5;010mhighest:', '{:.2f}'.format(target_acc[net.name]), '\033[0m')
...
def Demo(net_name, testloader):
    net = torch.load('results/weights/'+'model-'+net_name+'_demo.pth')
    test_acc = Test(net, testloader)
    print(('\033[38;5;011m'+net_name).ljust(30, ' '), '\t', '\033[0mtest acc:', '{:.2f}'.format(test_acc), ' ', )
...
def main():
    ...
    if mode == 'demo':
        # Demo('resnet18_pretrained', testloader)
        Demo('resnet18_pretrained', testloader)
        Demo('resnet50_pretrained', testloader)
        Demo('resnet18', testloader)
        Demo('resnet50', testloader)
        return
